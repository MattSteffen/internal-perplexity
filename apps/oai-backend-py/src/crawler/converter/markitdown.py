        ...
        try:
            ...
            if doc.source == "path":
                ...
                with tempfile.NamedTemporaryFile(delete=False, suffix=Path(doc.filename or "doc").suffix) as tmp:
                    if doc.source == "bytes":
                        if doc.bytes_data is None:
                            raise ValueError("bytes_data is required when source='bytes'")
                        tmp.write(doc.bytes_data)
                    else:  # fileobj
                        if doc.fileobj is None:
                            raise ValueError("fileobj is required when source='fileobj'")
