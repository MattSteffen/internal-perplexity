...
class MetadataExtractor:
    ...
    def _coerce_to_dict(self, result: Any) -> dict[str, Any]:
        ...
        if isinstance(result, str):
            try:
                return json.loads(result.strip())  # type: ignore[no-any-return]
            except json.JSONDecodeError:
                ...
                if start != -1 and end != -1 and end > start:
                    try:
                        return json.loads(result[start : end + 1])  # type: ignore[no-any-return]
                    except Exception:
                        pass
        ...
        if hasattr(result, "content"):
            try:
                return json.loads(str(result.content).strip())  # type: ignore[no-any-return]
            except Exception:
                pass
        if hasattr(result, "text"):
            try:
                return json.loads(str(result.text).strip())  # type: ignore[no-any-return]
            except Exception:
                pass
