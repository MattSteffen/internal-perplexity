...
def _create_base_schema(embedding_size: int) -> CollectionSchema:
...
def create_schema(
    embedding_size: int,
    user_metadata_json_schema: dict[str, Any] | None = None,
    library_context: str | None = None,
    pipeline_config: dict[str, Any] | None = None,
    permissions: dict[str, Any] | None = None,
) -> CollectionSchema:
    fields, functions = _create_base_schema(embedding_size)
    description_dict = create_description(
        fields,
        user_metadata_json_schema or {},
        library_context or "",
...
def create_index(client: MilvusClient) -> Any:
...
def create_description(
    ...
    Output:
      - A dictionary with three keys:
        - "description": The library context portion
        - "metadata_schema": The user metadata portion
        - "full_prompt": The full combined text suitable for `CollectionSchema.description`
    ...
    parts: list[str] = []
    metadata_schema_parts: list[str] = []

    # 1) Purpose/context
    description_text = f"Collection purpose and library context:\n{library_context}"
    ...
    # Build metadata_schema section (user metadata portion)
    metadata_schema_parts.append("User-defined metadata schema for the collection (JSON):")
    metadata_schema_parts.append(json.dumps(user_metadata_json_schema, indent=2))
    metadata_schema_parts.append("Note: The user metadata is stored as a JSON object in the 'metadata' field.")
    if not is_json_metadata:
        metadata_schema_parts.append("WARNING: The 'metadata' field does not appear to be a Milvus JSON field. " "JSON-path filtering requires a JSON field type.")
    ...
    result: dict[str, Any] = {
        "description": library_context,
        "metadata_schema": json.dumps(user_metadata_json_schema),
