...
# Expecting your project to provide this interface.
# LLM must implement: invoke(prompt, tools=None, response_format=None) -> Any
from ..llm.llm import LLM, schema_to_openai_tools
...
class MetadataExtractor:
    ...
    def __init__(self, llm: LLM, config: MetadataExtractorConfig) -> None:
        ...
        self.logger.info("Initialized MetadataExtractor")

    def run(self, markdown: str) -> MetadataExtractionResult:
        ...
        if self.config.include_benchmark_questions:
            questions = self.generate_benchmark_questions(
                markdown, self.config.num_benchmark_questions
            )
            ...
            if isinstance(response, str):
                ...
                if isinstance(response, dict) and "text" in response:
                    try:
                        ...
                        pass
                self.logger.warning("Unexpected response type for questions")
                return []
        except Exception as e:
            self.logger.error(f"Error generating benchmark questions: {e}")
            return []
    ...
    def _build_metadata_prompt(
        ...
        return EXTRACT_METADATA_PROMPT.replace("{{json_schema}}", schema_json).replace(
            "{{document_library_context}}", context or ""
    ...
    def _maybe_validate_with_jsonschema(
        ...
        try:
            ...
            self.logger.debug("jsonschema not installed; skipping validation")
        except Exception as e:
