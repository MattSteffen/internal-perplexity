...
class ImageDescriptionInterface(ABC):
    ...
    @abstractmethod
    def describe_image(self, image_data: bytes, image_ext: str, prompt: str = None) -> str:
        """
...
class OllamaImageDescriber(ImageDescriptionInterface):
    ...
    def describe_image(self, image_data: bytes, image_ext: str, prompt: str = None) -> str:
        ...
        try:
            ...
            print("called describer", response.json())
...
class PyMuPDFConverter(Converter):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.image_description_prompt = config.get("image_description_prompt", self.DEFAULT_IMAGE_PROMPT)
    ...
    def _extract_images_from_page(self, page: pymupdf.Page) -> List[ExtractedImage]:
        ...
        for img_index, img in enumerate(image_list):
            ...
            try:
                ...
                self.logger.error(f"Error extracting image {img_index} from page {page.number}: {e}")
                continue
               
        return extracted_images
    
    def _get_text_blocks_with_positions(self, page: pymupdf.Page) -> List[Dict[str, Any]]:
        """
    ...
    def _merge_content_by_position(self, text_blocks: List[Dict[str, Any]],
    ...
    def convert(self, filepath: str) -> str:
        ...
        try:
            self.logger.info(f"Converting {filepath} using PyMuPDF with comprehensive extraction")
            ...
            with pymupdf.open(filepath) as doc:
                ...
                for page_num in range(len(doc)):
                    ...
                    merged_content = self._merge_content_by_position(text_blocks, images, tables)
                    ...
                    for item in merged_content:
                        if item["type"] == "text":
                            ...
                            image_desc = item["image"].description or "[No description available]"
                            markdown_content.append(f"![Image Description]({item['image'].image_ext})\n*{image_desc}*\n\n")
                        elif item["type"] == "table":
                            markdown_content.append(f"**Table {item['table_index'] + 1}** ({item['rows']} rows × {item['cols']} cols)\n\n")
